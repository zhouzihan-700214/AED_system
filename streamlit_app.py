...
# Microsoft settings are read once by config.py using the proven v7 Secrets layout.
...
def render_microsoft_sign_in_gate() -> None:
    """Require the proven v7 Microsoft configuration and sign-in flow."""

    if not bool(getattr(config, "ONEDRIVE_CLOUD_ENABLED", False)):
        st.title("Microsoft OneDrive configuration required")
        st.error(
            "The application could not read the required [microsoft] settings. "
            "It will not fall back to any local, cached, or bundled AED workbook."
        )
    ...
    st.caption(
        "After sign-in, the official OneDrive workbook is downloaded before any AED page is opened."
    )
    st.stop()
...
def render_data_sync_control() -> None:
    ...
    with st.sidebar.expander("Data Source", expanded=False):
        ...
        if status.get("onedrive_enabled", False):
            st.success("Direct OneDrive transaction mode")
            st.caption(
                "Official workbook (read and write): "
